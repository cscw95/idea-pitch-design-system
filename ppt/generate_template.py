...
def add_badge(slide, author, team, x=11.00, y=0.18, w=1.90, h=0.66):
    sp = rect(slide, x, y, w, h, fill=BLUE)
    tf = _tf(sp)
    para(tf, author, size=14, weight='eb', color=WHITE, align=PP_ALIGN.CENTER, first=True)
    para(tf, team, size=10, color=WHITE, align=PP_ALIGN.CENTER)
    return sp


def add_header(slide, deck_name, section_label, author=None, team=None):
    ...
    if author:
        add_badge(slide, author, team or "")
...
# ── S2. 디자인 토큰 ───────────────────────────────────────
s = new_slide()
add_header(s, "Design System", "① 디자인 토큰", "발표자", "소속 팀")
...
tokens = [
    ...
    ("BLUE  #1273C4", "발표자 배지 · 포인트 액션", BLUE, WHITE),
...
# ── S3. 레이아웃 아나토미 ─────────────────────────────────
s = new_slide()
add_header(s, "Design System", "② 레이아웃 구조", "발표자", "소속 팀")
...
caption(s, 5.5, 0.48, 5.3, "① Header — 덱이름 | 섹션명 + 발표자 배지 + 룰",
...
# ── S4. 컴포넌트 라이브러리 ───────────────────────────────
s = new_slide()
add_header(s, "Design System", "③ 컴포넌트", "발표자", "소속 팀")
add_headline(s, "10개 컴포넌트의 조합만으로 모든 슬라이드를 구성")
...
# 4-3 Badge
add_badge(s, "김강호", "Access Tech.팀", x=9.4, y=2.5, w=1.9, h=0.66)
caption(s, 9.4, 3.9, 1.9, "Presenter Badge")
# 4-4 Emphasis band
...
s = new_slide()
add_header(s, "Idea Pitch", "① 상황 인식", "김강호", "Access Tech.팀")
...
s = new_slide()
add_header(s, "Idea Pitch", "② 방향성 제언", "김강호", "Access Tech.팀")
...
# ── S7. 빈 템플릿 A: 1 + 2 구성 ──────────────────────────
s = new_slide()
add_header(s, "Idea Pitch", "① [섹션명]", "[발표자]", "[소속 팀]")
